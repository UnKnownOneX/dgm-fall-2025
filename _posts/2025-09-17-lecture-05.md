...
abstract: >
  This lecture covers training algorithms for neural networks, focusing on gradient descent methods. We explore online, batch, and minibatch training modes, review calculus fundamentals including derivatives and gradients, and introduce the Adaline (Adaptive Linear Neuron) model.
---
...
<figure>
  <img src="{{ '/assets/img/notes/lecture-05/perceptron_recap.png' | relative_url }}" alt="Perceptron Recap" style="max-width:35%;"/>
  <figcaption><strong>Figure 1.</strong> Perceptron recap diagram, where:</figcaption>
</figure>
...
2. For every training epoch:
   ...
   - (a) Compute output (prediction) $$\hat{y}^{[i]} := \sigma\bigl( (\mathbf{x}^{[i]})^{\top} \mathbf{w} + b \bigr)$$

   - (b) Calculate error $$\hat{y}^{[i]} := \sigma\bigl( (\mathbf{x}^{[i]})^{\top} \mathbf{w} + b \bigr)$$

   - (c) Update parameters $$\mathbf{w} := \mathbf{w} + \mathrm{err} \times \mathbf{x}^{[i]}, \quad b := b + \mathrm{err}$$

### "On-line" mode (= SGD: Stochastic Gradient Descent)
...
### Minibatch mode (mix between on-line and batch)
...
2. For every training epoch:
   ...
   - (b) $ \nabla*{\mathbf{w}}\mathcal{L} = \bigl(y^{[i]} - \hat{y}^{[i]}\bigr)x^{[i]}, \quad
     \nabla*{b}\mathcal{L} = \bigl(y^{[i]} - \hat{y}^{[i]}\bigr) $

   - (c) $ \mathbf{w} := \mathbf{w} + \eta \times \bigl(-\nabla*{\mathbf{w}}\mathcal{L}\bigr), \quad
     b := b + \eta \times \bigl(-\nabla*{b}\mathcal{L}\bigr) $
...
2. For every training epoch:
   ...
   **B. For weight** $ \boldsymbol{j \in \{1, \ldots, m\}} $**:**
   - (b) $ \frac{\partial \mathcal{L}}{\partial w*{j}} =
     \bigl(y^{[i]} - \hat{y}^{[i]}\bigr)x*{j}^{[i]} $

   - (c) $ w*{j} := w*{j} + \eta \times
     \Bigl(-\frac{\partial \mathcal{L}}{\partial w\_{j}}\Bigr) $
...
<figure>
  <img src="{{ '/assets/img/notes/lecture-05/Image1.png' | relative_url }}" alt="Linear Regression Loss Surface" style="max-width:40%;"/>
  <figcaption><strong>Figure 2.</strong> Linear regression loss surface visualization.</figcaption>
</figure>
...
<figure>
  <img src="{{ '/assets/img/notes/lecture-05/Image2.png' | relative_url }}" alt="Learning Rate Effects" style="max-width:40%;"/>
  <figcaption><strong>Figure 3.</strong> Effects of learning rate on gradient descent convergence.</figcaption>
</figure>
...
<figure>
  <img src="{{ '/assets/img/notes/lecture-05/Image6.png' | relative_url }}" alt="Contour Plot" style="max-width:40%;"/>
  <figcaption><strong>Figure 4.</strong> Contour plot showing gradient descent updates perpendicular to contour lines.</figcaption>
</figure>
...
<figure>
  <img src="{{ '/assets/img/notes/lecture-05/Image7.png' | relative_url }}" alt="SGD Surface Plot" style="max-width:40%;"/>
  <figcaption><strong>Figure 5.</strong> Stochastic gradient descent surface plot showing noisy updates.</figcaption>
</figure>
...
<figure>
  <img src="{{ '/assets/img/notes/lecture-05/Image9.png' | relative_url }}" alt="ADALINE Architecture" style="max-width:40%;"/>
  <figcaption><strong>Figure 6.</strong> ADALINE (Adaptive Linear Neuron) architecture.</figcaption>
</figure>
