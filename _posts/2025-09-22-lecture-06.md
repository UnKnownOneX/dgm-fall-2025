...
- Automatic differentiation allows PyTorch to compute gradients **exactly and efficiently**.
- Computation graphs encode how values depend on each other and enable backpropagation.
- PyTorch uses **reverse-mode autodiff**, which is efficient for deep learning.
- Gradients are accumulated in `.grad` and must be reset each iteration.
- Dynamic computation graphs allow flexible model definitions using standard Python code.
...
- Explain what **automatic differentiation** is and why it is used in deep learning.
- Understand how **computation graphs** represent mathematical expressions.
- Apply the **chain rule** on computation graphs.
- Use PyTorch’s **autograd** system to compute gradients.
- Implement and debug a basic PyTorch training loop.
...
**Key Connection :**  
In PyTorch, computation graphs are **built dynamically during the forward pass (define-by-run)**.  
When `backward()` is called, PyTorch traverses this graph in reverse order and applies the chain rule automatically.
...
**Why Reverse Mode?:**  
Deep learning models typically have **millions of parameters** but a **single scalar loss**.  
Reverse-mode autodiff computes gradients with respect to all parameters in one backward pass, making it far more efficient than forward-mode for neural networks.
...
**Takeaway:**  
The forward pass builds the computation graph, and `loss.backward()` uses that graph to compute gradients.  
The optimizer then updates parameters, completing one gradient descent step.
