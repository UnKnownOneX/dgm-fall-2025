...
For example, to predict the word "is" in "The capital of France \_\_\_ Paris.", the model must:
...
### Why Does Scale Create Emergent Abilities?

The lecture emphasizes that while test loss improves smoothly as we scale
model size, data, and compute (as seen in Kaplan et al. 2021), the actual
*capabilities* of the model do not always grow smoothly. Instead, some
abilities appear suddenly once the model crosses a certain scale.

This happens because:
- Larger models can represent higher-order dependencies in text.
- More data exposes the model to more rare reasoning patterns.
- MLE forces the model to model global structure, not just local token
  frequencies.
- Transformers act as meta-learners, inferring tasks from context rather
  than explicit supervision.

This contextual bridge matches the lecture slides on scaling and emergence
(pages 12–17). :contentReference[oaicite:0]{index=0}
...
3. **Factual structure understanding**: To fill in “The capital of France \_\_\_ Paris”, model must:
...
$\text{KL}(P_{\text{data}} \mid P_{\theta})= E_{x \sim P_{\text{data}}}[\log P_{\theta}(x)] + \text{const}$

**Implications of this KL direction**

- The model must **place probability mass on _all_ observed sequences**, even if some are incoherent or low-quality.
- This can lead to:
  - **Repetition and genericity**  
    e.g. outputs like “_The man said the man said…_”.
...
### 3.2 What MLE _Does Not_ Provide
...
So, a pure MLE-trained LM is a very good _next-token predictor_, but not
...
$H_{t} = - \sum_v P(x_{t} = v \mid x_{<t}) \log P(x_{t} = v \mid x_{<t})$ \\
$= E_{v} [- \log P(x_{t} = v \mid x_{<t})]$
...
> _Do we want the model to be low-entropy or high-entropy in its predictions?_
...
$\log p(x \mid \theta)$ 
$= E_{z \sim q} [\log p(x,z \mid \theta)] + H(q) + KL(q(z \mid x) || p(z \mid x, \theta))$

This yields the **Evidence Lower Bound (ELBO)**:

$\log p(x \mid \theta) \ge E_{z \sim q}[\log p(x,z \mid \theta)] + H(q)$
...
explicit entropy term** for \(P\_\theta(x)\).
...
$\hat{\theta_{MLE}} = argmax_{\theta} E_{x \sim P_{data}} [\log P_{\theta}(x)]$
...
$\hat{\theta} = argmax_{\theta} E_{x \sim P_{data}} [\log P_{\theta} (x)]$ 
$+ \lambda \sum_{t} H[P_{\theta}(x_{t} \mid x_{<t})]$
...
\begin{cases}
1 - \epsilon, & y = 1 \\
\displaystyle \frac{\epsilon}{|V| - 1}, & y = 0
\end{cases}
$$

- $V$: vocabulary size
- $\epsilon$: small smoothing constant (e.g. 0.1)
...
- **Radford et al. (2018)** — _Improving Language Understanding by Generative Pre-Training_ (GPT-1)
- **Brants et al. (2007)** — _Large Language Models in Machine Translation_
- **Holtzman et al. (2020)** — _The Curious Case of Neural Text Degeneration_
- **Kaplan et al. (2021)** — _Scaling Laws for Neural Language Models_
- **Understanding LLMs (2024)** — _Overviews of LLM training to inference pipeline_
- **LLM360 / TxT360**
